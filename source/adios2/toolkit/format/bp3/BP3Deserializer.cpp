...
void BP3Deserializer::ParseMetadataNew(const BufferSTL &bufferSTL, IO &io)
{
    size_t steps;
...
void BP3Deserializer::ParseMetadataIndex(const BufferSTL &bufferSTL)
{
    ...
    while (position < bufferSize)
    {
        ...
        const uint64_t currentStep = ReadValue<uint64_t>(buffer, position);
        const uint64_t mpiRank = ReadValue<uint64_t>(buffer, position);
        const uint64_t pgIndexStart = ReadValue<uint64_t>(buffer, position);
        ptrs.push_back(pgIndexStart);
        const uint64_t variablesIndexStart = ReadValue<uint64_t>(buffer, position);
        ptrs.push_back(variablesIndexStart);
        const uint64_t attributesIndexStart = ReadValue<uint64_t>(buffer, position);
        ptrs.push_back(attributesIndexStart);
        const uint64_t endptrval = ReadValue<uint64_t>(buffer, position);
...
void BP3Deserializer::ParsePGIndexPerStep(const BufferSTL &bufferSTL, const IO &io, size_t step)
{
    const auto &buffer = bufferSTL.m_Buffer;
    ...
    m_MetadataSet.DataPGCount = m_MetadataSet.DataPGCount+ReadValue<uint64_t>(buffer, position);
...
void BP3Deserializer::ParseVariablesIndexPerStep(const BufferSTL &bufferSTL, IO &io, size_t step)
{
...
void BP3Deserializer::ParseAttributesIndexPerStep(const BufferSTL &bufferSTL, IO &io, size_t step)
{
    auto lf_ReadElementIndex = [&](IO &io, const std::vector<char> &buffer,
                                   size_t position) {
