...
protected:
    ...
    /**
     * DataTypes mapping in BP4 Format
     */
    enum DataTypes
    {
    ...
    /** Returns the proper derived class for BP4Operation based on type
     ...
     * @return
     */
    std::string ReadBP4String(const std::vector<char> &buffer, size_t &position,
                              const bool isLittleEndian = true) const noexcept;
